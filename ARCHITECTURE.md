...
## 13. Fixture Data Layer (interim)

The UI screens from the client's approved mockup are built and working, but Supabase is not connected yet. Until it is, the screens read from a typed fixture layer in `lib/fixtures/`:

- `lib/fixtures/bikes.ts` — the bike records (the mockup's own M01187–M01196 sample rows, extended programmatically to a pagination-worthy set), plus status→colour mapping.
- `lib/fixtures/dashboard.ts` — dashboard aggregations **derived from the same bike array** (counts by status, insurer, upliftment stage, location) rather than separately hardcoded numbers, so the dashboard stays internally consistent and mirrors the shape the real SQL aggregation will take.

This is deliberately a thin, single-purpose seam: replacing it means changing `lib/fixtures/*` (and later routing those calls through `services/*`) without touching the presentational components, which receive plain typed props.

Two things are already **real**, not mocked, because the libraries are installed and the behavior is genuinely testable:

- **QR codes** — generated by `qrcode` from the bike's actual record URL (`lib/qr/generate.ts`). Scannable today; the domain comes from `NEXT_PUBLIC_SITE_URL`.
- **Excel parsing** — the Data Import screen really parses uploaded `.xlsx` with SheetJS: worksheet detection, header extraction, row counts. The final mapping/validation/write steps are stubbed pending the database, and the UI says so explicitly rather than implying an import occurred.

Trend percentages on the dashboard stat cards ("+12% vs last month") remain static labels — there is no fixture time-series to compute them from, and inventing one would be misleading.
